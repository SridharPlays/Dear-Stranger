...
// --- Main App Screens ---
const PhoneHomeScreen = ({ setCurrentScreen }) => (
...
const PhotosApp = ({ setCurrentScreen, setSelectedPhoto }) => (
  <View style={styles.appContainer}>
    <AppHeader title="Gallery" onBack={() => setCurrentScreen("home-screen")} />
    <View style={styles.photoGrid}>
      <TouchableOpacity
        onPress={() => {
          setSelectedPhoto(require("../assets/images/ticket.png"));
      ...
      <TouchableOpacity
        onPress={() => {
          setSelectedPhoto(
            require("../assets/images/mysterious.jpg")
          );
          setCurrentScreen("photo-view");
        }}
      >
...
const PhotoView = ({ setCurrentScreen, photo }) => (
  <View style={styles.photoViewContainer}>
    ...
    <Image
      // This now works for both network images and local images
...
const NotesApp = ({ setCurrentScreen }) => (
  <View style={styles.appContainer}>
    <AppHeader title="Notes" onBack={() => setCurrentScreen("home-screen")} />
    <ScrollView style={{ padding: 8 }}>
      <TouchableOpacity
...
// --- Replace the old EndScreen component with this one ---

const EndScreen = ({ title, subtitle, onRestart, sequelTeaser }) => {
  const initialFade = useRef(new Animated.Value(0)).current;
  const teaserLine1Fade = useRef(new Animated.Value(0)).current;
  const teaserLine2Fade = useRef(new Animated.Value(0)).current;
  const teaserLine3Fade = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    // Base sequence: fade in the title and wait.
    const animationSequence = [
      ...
      Animated.delay(4000), // Hold the end screen message for 4 seconds
    ];

    // If a sequel teaser is provided, add more animations
    if (sequelTeaser) {
      animationSequence.push(
        Animated.timing(initialFade, {
          ...
          duration: 1500,
          useNativeDriver: true,
        }), // Fade out the original message
        Animated.delay(1500),
        // --- The Sequel Teaser ---
        Animated.timing(teaserLine1Fade, {
          toValue: 1,
          duration: 2000,
          useNativeDriver: true,
        }), // "A few days later..."
        ...
        Animated.delay(1000),
        Animated.timing(teaserLine2Fade, {
          toValue: 1,
          duration: 2500,
          useNativeDriver: true,
        }), // "Riya was kidnapped."
        Animated.delay(3500),
        Animated.timing(teaserLine3Fade, {
          toValue: 1,
          duration: 2000,
          useNativeDriver: true,
        }), // "ACT II..."
        Animated.delay(5000) // Hold the final teaser
      );
    }

    Animated.sequence(animationSequence).start(() => {
      // After all animations are complete, restart the game
      onRestart();
    });
  }, []);
  ...
  return (
    <View style={styles.endScreenContainer}>
      {/* Initial Ending Message */}
      <Animated.View style={{ opacity: initialFade, alignItems: "center" }}>
        <Text style={styles.endScreenTitle}>{title}</Text>
        <Text style={styles.endScreenSubtitle}>{subtitle}</Text>
        {/* The "Play Again" button is now hidden when there's a sequel teaser */}
        {!sequelTeaser && (
      ...
      {/* The Sequel Teaser Text (only renders if sequelTeaser is provided) */}
      {sequelTeaser && (
        <View style={styles.sequelContainer}>
          <Animated.Text
            style={[styles.sequelText, { opacity: teaserLine1Fade }]}
          >
            {sequelTeaser.line1}
          </Animated.Text>
          <Animated.Text
            style={[styles.sequelTextDramatic, { opacity: teaserLine2Fade }]}
          >
            {sequelTeaser.line2}
          </Animated.Text>
          <Animated.Text
            style={[styles.sequelActTitle, { opacity: teaserLine3Fade }]}
          >
            {sequelTeaser.line3}
          </Animated.Text>
        </View>
...
const IntroAndMenuScreen = ({ onStartGame }) => {
  ...
  const episodeTranslateY = useRef(new Animated.Value(0)).current;
  ...
  useEffect(() => {
    if (!introPlayed.current) {
      Animated.sequence([
        Animated.timing(headphonesFade, {
          toValue: 1,
          duration: 2000,
          useNativeDriver: true,
        }),
        ...
        Animated.delay(2000),
        Animated.parallel([
          Animated.timing(episodeTranslateY, {
            toValue: 80,
            duration: 1500,
            useNativeDriver: true,
          }),
  ...
  return (
    <View style={introStyles.container}>
      ...
      <Animated.View
        style={[
          introStyles.backgroundImageContainer,
          { opacity: backgroundOpacity },
        ]}
      >
        <ImageBackground
          source={require("../assets/images/logo_background.jpg")}
          style={introStyles.fullScreenImage}
          resizeMode="cover"
        >
          <Animated.View
            style={[
              introStyles.episodeContainer,
              {
                opacity: episodeFade,
          ...
          >
            <Text style={introStyles.episodeTitle}>Act I: The Prequel</Text>
          </Animated.View>
        </ImageBackground>
      </Animated.View>
      <Animated.View
        style={[introStyles.centered, { opacity: headphonesFade }]}
      >
        <Text style={introStyles.headphonesText}>
          Wear headphones for the best experience
        </Text>
      </Animated.View>
      <Animated.View style={[introStyles.centered, { opacity: studioFade }]}>
        <Text style={introStyles.studioText}>Power Angels Presents</Text>
      </Animated.View>
      <Animated.View
        style={[introStyles.menuContainer, { opacity: menuButtonsFade }]}
      >
        <TouchableOpacity
          ...
          disabled={!menuActive}
        >
          <Text style={introStyles.buttonText}>START</Text>
        </TouchableOpacity>
      </Animated.View>
    </View>
  );
...
const UnlockedApp = ({ onRestart }) => {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      case "home-screen":
        return <PhoneHomeScreen setCurrentScreen={setCurrentScreen} />;
      case "messages-app":
        return (
          <MessagesApp
      ...
      case "end-screen-good":
        return (
          <EndScreen
            ...
            onRestart={onRestart}
            sequelTeaser={{
              line1: "A few days later...",
              line2: "Riya was kidnapped.",
              line3: "ACT II: THE ECHOING SILENCE",
            }}
          />
        );
...
export default function App() {
  ...
  useEffect(() => {
    const manageBGM = async () => {
      ...
      if (currentScreen === "intro") {
        soundFile = require("../assets/music/mystery_bgm.mp3");
      } else if (currentScreen === "story") {
        soundFile = require("../assets/music/tense_bgm.mp3");
      }
      if (!soundFile) return;
      try {
  ...
  useEffect(() => {
    let batteryTimer;
    if (currentScreen === "story" && phoneUnlocked === true) {
  ...
  }, [phoneUnlocked, currentScreen]);

  const restartGame = () => {
    setBatteryLevel(15);
    setPhoneUnlocked(false);
    setGameKey((prevKey) => prevKey + 1);
    setCurrentScreen("intro"); // Go back to intro on restart
  };
  ...
  switch (currentScreen) {
    ...
    default:
      return (
        <IntroAndMenuScreen onStartGame={() => setCurrentScreen("story")} />
      );
  }
}

// Separate styles for Intro screen to avoid conflicts
...
// Original styles for the main game
const styles = StyleSheet.create({
  ...
  // --- Add these to your main 'styles' object ---
  ...
  sequelActTitle: {
    color: "#F87171", // A dramatic red
    fontSize: 28,
    fontWeight: "600",
    textAlign: "center",
    position: "absolute",
    marginTop: 80, // Position it below the other text
  },
});
